...
<?php
	...
	if( isset($_POST['check']) ){ //verifie qu'on a cliqué sur le bouton de connexion
		...
		$email= $_POST["email"];
        $mdp= $_POST["mdp"];
        $changedpasswd= $_POST["mdp_change"];
		$res= mysqli_query($link,"UPDATE utilisateur SET passwd='$changedpasswd' WHERE email='$email' && passwd='$mdp'");
		echo $email;
		echo $mdp;
		$login_tableau=	mysqli_fetch_assoc($res);
		//si l'internaute a tape le bon mail et mdp -> connexion
		if( $login_tableau != NULL){ 
			foreach ($login_tableau as $login){
				$_SESSION["login"] = $login;
				echo "<p>Ravi de vous revoir ". $_SESSION["login"] ."</p>";
			}
		}else{
			echo "<p>Mot de passe ou mail incorrect</p>";
        }
