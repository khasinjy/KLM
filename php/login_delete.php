...
<?php
	...
	if( isset($_POST['check']) ){ //verifie qu'on a cliqué sur le bouton de connexion
		...
		$email= $_POST["email"];
		$mdp= $_POST["mdp"];
		$res= mysqli_query($link,"UPDATE utilisateur SET del=1 WHERE email='$email' && passwd='$mdp'");
		echo $email;
		echo $mdp;
		$login_tableau=	mysqli_fetch_assoc($res);
		//si l'internaute a tape le bon mail et mdp -> connexion
		if( $login_tableau != NULL){ 
			echo "<script>alert('Failure!');location.href='myaccount.php';</script>";
		}else{
			echo "<script>alert('Success!');location.href='logout.php';</script>";
		}
